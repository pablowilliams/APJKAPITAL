...
export default function Navbar({ openSection, onNavigate }: Props) {
  ...
  return (
    <>
      <motion.nav
        ...
        className="fixed top-0 left-0 right-0 z-50 bg-[#050506]/85 backdrop-blur-xl border-b border-white/[0.04]"
      >
        <div className="max-w-[1400px] mx-auto px-6 md:px-16 lg:px-24 flex items-center justify-between h-16">
          ...
          {/* Desktop nav — section buttons */}
          <div className="hidden lg:flex items-center gap-1">
            {sections.map((s) => (
              <button
                key={s.id}
                onClick={() => handleNav(s.id)}
                className={`relative px-3.5 py-1.5 text-[13px] tracking-[0.03em] rounded-md transition-all duration-200 ${
                  openSection === s.id
              ...
              {sections.map((s, i) => (
                <motion.button
                  ...
                  className={`text-left text-[24px] font-display font-medium py-3 transition-colors ${
                    openSection === s.id ? 'text-gold' : 'text-zinc-400'
                  }`}
                >
                  {s.label}
                </motion.button>
